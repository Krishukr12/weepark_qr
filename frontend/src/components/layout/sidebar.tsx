...
export function BrandLogo({ className, onDark = false }: { className?: string; onDark?: boolean }) {
  return (
    <img
      src="/icon-dark.png"
      alt="WeePark logo"
      className={cn('size-8 select-none', onDark ? 'invert' : 'dark:invert', className)}
      draggable={false}
    />
  );
}
  ...
  return (
    <div className="flex h-full flex-col bg-sidebar">
        ...
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              <item.icon className={cn('relative z-10 size-4.5', isActive && 'text-brand')} />
      ...
      <div className="border-t border-sidebar-border p-4">
        <p className="text-xs text-sidebar-foreground/50">WeePark v1.0 · Smart Parking</p>
      </div>
    </div>
  );
}
