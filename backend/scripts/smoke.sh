...
LOGIN=$(curl -s -X POST "$BASE/auth/login" -H 'Content-Type: application/json' \
  -d '{"email":"admin@weepark.io","password":"Admin@1234"}')
...
VALET=$(curl -s -X POST "$BASE/valets" -H "$AUTH" -H 'Content-Type: application/json' \
  -d "{\"name\":\"Smoke Valet\",\"email\":\"valet$STAMP@weepark.io\",\"phone\":\"9800000001\",\"password\":\"Valet@1234\",\"siteIds\":[\"$SITE_ID\"]}")
...
ORG=$(curl -s -X POST "$BASE/organizations" -H "$AUTH" -H 'Content-Type: application/json' \
  -d "{\"name\":\"Smoke Org $STAMP\",\"companyName\":\"Smoke Pvt Ltd\",\"adminName\":\"Org Admin\",\"adminEmail\":\"orgadmin$STAMP@weepark.io\",\"adminPhone\":\"9800000002\",\"parkingAllocation\":10}")
...
echo "lookup found=$(echo "$LOOKUP" | json '.data.found')"

step "public: park vehicle"
PARK=$(curl -s -X POST "$BASE/public/parking/sites/$SITE_CODE/park" -H 'Content-Type: application/json' \
  -d "{\"vehicleId\":\"$VEH_ID\"}")
ENTRY_ID=$(echo "$PARK" | json '.data.id')
echo "ok: entry parked status=$(echo "$PARK" | json '.data.status')"

step "public: entry status"
curl -s -o /dev/null -w "entry status http=%{http_code}\n" "$BASE/public/parking/entries/$ENTRY_ID"

step "public: request pickup (GET MY CAR)"
PICKUP=$(curl -s -X POST "$BASE/public/pickups/request" -H 'Content-Type: application/json' \
  -d "{\"parkingEntryId\":\"$ENTRY_ID\"}")
...
VLOGIN=$(curl -s -X POST "$BASE/auth/login" -H 'Content-Type: application/json' \
  -d "{\"email\":\"valet$STAMP@weepark.io\",\"password\":\"Valet@1234\"}")
...
step "verify parking entry completed"
ENTRY=$(curl -s -H "$AUTH" "$BASE/parking/$ENTRY_ID")
echo "entry status=$(echo "$ENTRY" | json '.data.status')"
